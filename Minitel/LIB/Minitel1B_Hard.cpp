...
Minitel::Minitel(xUART& serial) : mySerial(serial) {
...
uint24_t Minitel::serial_begin(int b) { 
  ...
  mySerial.baudRate = (uint24_t)b;
	mySerial.dataBits = 7;
	mySerial.stopBits = 1;
	mySerial.parity = PAR_EVPARITY;
	mySerial.hwFlowControl = 0;
	mySerial.interrupts = UART_IER_RECEIVEINT;
	//printf("bauds : %d\r\n",  mySerial.baudRate);
...
char Minitel::serial_read() {
  	return uart1_read();
}

void Minitel::close_serial() {
    if(oldvector != NULL) {  
      close_UART1();      
      //mos_setintvector(UART1_IVECT, (void (*)())oldvector);
      mos_setintvector(UART1_IVECT, reinterpret_cast<void (*)()>(oldvector));
    }
}

void Minitel::serial_flush() {
  	uart1_flush();
}
...
void Minitel::zone(int lig, int col, int len, const char * s, int color)
{
    zoneInsert(head, lig, col, len, s, color);  
    /*moveCursorXY(col, lig);
...
//Goto first field
void Minitel::zoneBegin()
{
  if(head == NULL) return;
  //iter = first;         
  moveCursorXY(head->col, head->lig);  
}

void Minitel::sendPage(uint8_t * page, long len) {
  	for(long i = 0; i < len; i++) {
      writeByte(page[i]);
    }
}

//Reset all zone field
void Minitel::resetZone()
{    
  zoneFree();  
}

//Get nb zone
int Minitel::getNbZone()
{
  iter = head; 
  int c = 0;
  while (iter != NULL) {
...
void Minitel::gotoZone(int n)
{
  ...
  iter = head;
  for (i = 1; (i <= c && iter != NULL); i++)
  {
      if (i == n) {
        moveCursorXY(iter->col-1, iter->lig);
        break;
      }
      iter = iter->next;
  }
...
bool Minitel::loadPage(const char* filename)
{       
  long size;          
  FILE *f;
...
void Minitel::writeByte(uint8_t b) {
  ...
  serial_write(b);  // Envoi de l'octet sur le port série
}
/*--------------------------------------------------------------------*/

void Minitel::writeWord(uint16_t w) {
...
void Minitel::writeCode(unsigned long code) {
  ...
  if (code > 0x00FFFFFF) writeByte((uint8_t) (code >> 24));
  if (code > 0x0000FFFF) writeByte((uint8_t) (code >> 16));
  if (code > 0x000000FF) writeByte((uint8_t) (code >> 8));
  writeByte((uint8_t) code);
}
/*--------------------------------------------------------------------*/

uint8_t Minitel::readByte() {
...
int Minitel::searchSpeed() {
  ...
  do {    
    close_serial();
    serial_begin(SPEED[i]);    
    if (i++ > 3) { i = 0; }    
    speed = currentSpeed(); 
    //printf("searchSpeed: %d\r\n", speed);   
  } while (speed < 0);
...
/*--------------------------------------------------------------------*/
void Minitel::emptyLines(int firstline, int nbLines)
{
  noCursor();
  moveCursorXY(1,firstline);
  clearScreenFromCursor();
  for (int j=0; j < nbLines; j++) {
    attributs(CARACTERE_BLEU);
    print(".");
    repeat(39);
    attributs(CARACTERE_BLANC);
  }
...
/*--------------------------------------------------------------------*/
void Minitel::newPage(const char * titre) {
  newScreen();
  println(titre);
  for (int i=1; i<=40; i++) {
    writeByte(0x5F);
...
uint8_t Minitel::modeMixte() {  // Voir p.144
...
uint8_t Minitel::modeVideotex() {  // Voir p.144
...
uint8_t Minitel::standardTeleinformatique() {  // Voir p.144
...
uint8_t Minitel::standardTeletel() {  // Voir p.144
...
void Minitel::attributs(uint8_t attribut) {
...
void Minitel::print(const char * chaine) {
...
*/
  ...
  while (i < strlen(chaine)) {
    unsigned long code = (uint8_t) chaine[i++]; //chaine.charAt(i++);
    if (code < SP) code = 0;
      ...
      code = (code << 8) + (uint8_t) chaine[i++]; //chaine.charAt(i++);
...
void Minitel::println(const char * chaine) {
  print(chaine);
...
bool Minitel::available()
{
  return (uart1_kbhit() ? true : false);
}
...
void Minitel::printChar(char caractere) {
  ...
  uint8_t charByte = getCharByte(caractere);
...
void Minitel::printSpecialChar(uint8_t b) {
...
uint8_t Minitel::getCharByte(char caractere) {
  ...
  char caracteres[] = "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_xabcdefghijklmnopqrstuvwxyz{|}";
  return (uint8_t) lastIndexOf(caracteres, &caractere);
}
/*--------------------------------------------------------------------*/
char * Minitel::getString(unsigned long code, char* str) {
...
void Minitel::graphic(uint8_t b, int x, int y) {
  moveCursorXY(x,y);
  graphic(b);
}
/*--------------------------------------------------------------------*/

void Minitel::graphic(uint8_t b) {
...
unsigned long Minitel::getKeyCode(bool unicode) {
  ...
  if (code == 0x19)
  {  // SS2
    while (!available());  // Indispensable
    code = (code << 8) + readByte();
    // Les diacritiques (3 codes)
    if ((code == 0x1941) || (code == 0x1942) || (code == 0x1943) || (code == 0x1948) || (code == 0x194B))
  ...
  // Pour revenir au clavier vidéotex standard  : Fnct C + V
    ...
    delayms(20);  // Indispensable. 0x1B seul correspond à la touche Esc,
...
uint8_t Minitel::extendedKeyboard() {
  // Commande
...
uint8_t Minitel::standardKeyboard() {
  // Commande
  writeBytesPRO(3);                   // 0x1B 0x3B
  writeByte(STOP);                    // 0x6A
  writeByte(CODE_RECEPTION_CLAVIER);  // 0x59
...
uint8_t Minitel::echo(bool commande) {  // Voir p.81, p.135 et p.156
  // Fonction modifiée par iodeo sur GitHub en octobre 2021
...
uint8_t Minitel::aiguillage(bool commande, uint8_t emetteur, uint8_t recepteur) {  // Voir p.135
  // commande peut prendre comme valeur :
  // true, false
...
uint8_t Minitel::statusAiguillage(uint8_t module) {  // Voir p. 136
...
uint8_t Minitel::connexion(bool commande) {  // Voir p.139
...
uint8_t Minitel::reset() {  // Voir p.145
  // Commande
  writeBytesPRO(1);  // 0x1B 0x39
...
bool Minitel::isValidChar(uint8_t index) {
...
/*--------------------------------------------------------------------*/
/*
bool Minitel::isDiacritic(unsigned char caractere) {  // Obsolète depuis le 26/02/2023
  String diacritics = "àâäèéêëîïôöùûüçÀÂÄÈÉÊËÎÏÔÖÙÛÜÇ";
...
bool Minitel::isVisualisable(unsigned long code) {
  // Fonction proposée par iodeo sur GitHub en février 2023
...
unsigned long Minitel::identificationBytes() {  // Voir p.138
  while (!available());  // On attend que le port soit sur écoute.
...
int Minitel::workingSpeed() {
  int bauds = -1;
  //while (!available());  // On attend que le port soit sur écoute. on enleve les commentaires si on utilise seul minitel.changeSpeed() 
  ticker = 0;
  unsigned long duree = 0;
  unsigned long trame = 0;  // 32 bits = 4 octets
  
  unsigned long time = ticker; 
  timer0_begin(4608, 4);	
  
  // On se donne 1000 ms pour récupérer une trame exploitable
  while ((trame >> 8 != 0x1B3A75) && (duree < 1000)) {  //0x1B3A75
    if (available() /*> 0*/) {
      trame = (trame << 8) + readByte();      
      //printf("%lX\r\n", trame);      
    }
  ...
  switch (trame) {
    ...
    case 0x1B3A757F : bauds = 9600; break;  // 0x1B3A757F Pour le Minitel 2 seulement
  }
...
uint8_t Minitel::workingMode() {  // Voir p.143
  ...
  while (!available());  // On attend que le port soit sur écoute.
  unsigned long trame = 0;  // 32 bits = 4 octets  
  while (trame >> 8 != 0x1B3A73) {  // (0x1B3A73) PRO2 (0x1B,0x3A), REP_STATUS_FONCTIONNEMENT (0x73)  
    if (available() > 0) {
      trame = (trame << 8) + readByte();
      //Serial.println(trame, HEX);   
      //printf("%lX\r\n", trame);         
  ...
  return (uint8_t) trame;
}
/*--------------------------------------------------------------------*/

uint8_t Minitel::workingKeyboard() {  // Voir p.142
...
uint8_t Minitel::workingAiguillage(uint8_t module) {  // Voir p.136
 ...
 //***** while (!mySerial);  // On attend que le port soit sur écoute.
  unsigned long trame = 0;  // 32 bits = 4 octets
  while (trame != (0x1B3B63UL << 8 | module)) {  // PRO3 (0x1B,0x3B), FROM (0x63), code réception ou émission du module
    if (available() > 0) {
      trame = (trame << 8) + readByte();
      //Serial.println(trame, HEX);
      printf("%lX\r\n", trame);
...
uint8_t Minitel::workingModem() {  // Voir p.126
  // Fonction proposée par iodeo sur GitHub en octobre 2021
...
/*--------------------------------------------------------------------*/
bool Minitel::fileExt(const char * filename)
{
  const char *ext = strrchr(filename, '.');
  if (!ext) {
      return false;
  } else {
      if ((strcmp(ext+1, "vdt") == 0) || (strcmp(ext+1, "VDT") == 0))
        return true;      
  }
  return false;
}

zone_t * Minitel::zoneInsert(zone_t *head, int lig, int col, int len, const char * s, int color)
{        
    zone_t *cursor = head;
    ...
    // create a new node 
    zone_t* new_node =  createZone(NULL, lig, col, len, s, color);
    cursor->next = new_node;

    return head;
     // Create new node to insert 
    /*zone_t *field = (zone_t *)calloc (1, sizeof(zone_t));
    if (field == NULL) {
...
zone_t * Minitel::createZone(zone_t *next, int lig, int col, int len, const char * s, int color)
{    
     /* Create new node to insert */
    zone_t *new_node  = (zone_t *)calloc (1, sizeof(zone_t));
    if (new_node  == NULL) {
        fprintf (stderr, "error: virtual memory exhusted.\n");
        exit (2);
    }
...
zone_t * Minitel::prependZone(int lig, int col, int len, const char * s, int color)
{    
    node* new_node = createZone(head, lig, col, len, s, color);
    head = new_node;
    firstZone = new_node;
    return head;
}

void Minitel::zoneFree()
{ 
   /* Free the list */
    iter = head;
    while (iter->next) {
