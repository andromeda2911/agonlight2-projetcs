...
// On peut les afficher directement en utilisant printSpecialChar(uint8_t b) :
#define LIVRE              0x23
#define DOLLAR             0x24
#define DIESE              0x26
...
// printSpecialChar(uint8_t b) n'aura donc aucun effet ici.
...
// en utilisant attributs(uint8_t attribut).
...
public:
  ...
  void emptyLines(int firstline, int nbLines); //Empty lines with '.'
  ...
  uint8_t pageMode();      // Mode page
  uint8_t scrollMode();    // Mode rouleau
  uint8_t modeMixte();     // Mode Vidéotex => Mode Mixte 80 colonnes (Aucun caractère semi-graphique (jeu G1) n'est visualisable)
  uint8_t modeVideotex();  // Mode Mixte => Mode Vidéotex 40 colonnes

  // Standards
  uint8_t standardTeleinformatique();  // Standard Télétel => Standard Téléinformatique 80 colonnes (Possibilités de programmation moins étendues)
  uint8_t standardTeletel();           // Standard Téléinformatique => Standard Télétel (inclut les modes Vidéotex et Mixte)

  // Contenu
  void attributs(uint8_t attribut);
  void print(const char * chaine);  // UTF-8 => Codes Minitel
  void println(const char * chaine);
  void println();
  ...
  void printSpecialChar(uint8_t b);  // Caractère du jeu G2. Voir plus haut, au niveau de 1.2.3, les constantes possibles.
  uint8_t getCharByte(char caractere);
  char * getString(unsigned long code, char * str);  // Unicode => UTF-8
  int getNbBytes(unsigned long code);  // À utiliser en association avec getconst char *(unsigned long code) juste ci-dessus.
  void graphic(uint8_t b, int x, int y);  // Jeu G1. Voir page 101. Sous la forme 0b000000 à 0b111111 en allant du coin supérieur gauche au coin inférieur droit. En colonne x et rangée y.
  void graphic(uint8_t b);  // Voir la ligne ci-dessus.
  ...
  uint8_t smallMode();  // Mode minuscules du clavier
  uint8_t capitalMode();  // Mode majuscules du clavier
  uint8_t extendedKeyboard();  // Clavier étendu
  uint8_t standardKeyboard();  // Clavier standard
  uint8_t echo(bool commande);  // Active ou désactive l'écho à l'écran de ce qui est tapé au clavier
  
  // Protocole
  uint8_t aiguillage(bool commande, uint8_t emetteur, uint8_t recepteur);
  uint8_t statusAiguillage(uint8_t module);
  uint8_t connexion(bool commande);
  uint8_t reset();
  
  bool loadPage(const char* filename);
  void sendPage(uint8_t * page, long len);
  void serial_flush();  
  void end();
  
  void zone(int lig, int col, int len, const char * s, int color);
  void showZone();
  void zoneBegin();  
  void resetZone();
  int getNbZone();
  void gotoZone(int n);
  zone_t * prependZone(int lig, int col, int len, const char * s, int color);
private: 
  xUART mySerial; 
  zone_t *head;
  zone_t *iter;
  ...
  uint8_t currentSize = GRANDEUR_NORMALE;
  bool isValidChar(uint8_t index);
  // bool isDiacritic(unsigned char caractere);  // Obsolète depuis le 26/02/2023
  bool isVisualisable(unsigned long code);
  ...
  int workingSpeed();
  uint8_t workingStandard(unsigned long sequence);
  uint8_t workingMode();
  uint8_t workingKeyboard();
  uint8_t workingAiguillage(uint8_t module);
  uint8_t workingModem();
  
  unsigned long getCursorXY();

  bool fileExt(const char * filename);
  zone_t * zoneInsert(zone_t *head, int lig, int col, int len, const char * s, int color);
  void zoneFree();
  zone_t * createZone(zone_t *head, int lig, int col, int len, const char * s, int color);
